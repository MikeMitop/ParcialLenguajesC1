...
{
  "\"end of file\"", "error", "\"invalid token\"", "NUMBER", "SQRT",
  "EOL", "$accept", "calc", "expr", YY_NULLPTR
};
...
#line 49 "calculadora.y"
                { 
        double resultado = sqrt_newton((yyvsp[0].val)); 
        if (resultado != -1.0) {
            printf("La raiz cuadrada de %.2f es: %f\n", (yyvsp[0].val), resultado); 
        }
    }
